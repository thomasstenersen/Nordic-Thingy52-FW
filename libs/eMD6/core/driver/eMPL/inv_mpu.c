...
int mpu_write_mem(unsigned short mem_addr, unsigned short length,
        unsigned char *data)
{
//    SEGGER_RTT_printf(0, "mpu_write_mem: mem_addr=%x - length=%x - data=%x\r\n", mem_addr, length, data);
    unsigned char tmp[2];
...
int mpu_read_mem(unsigned short mem_addr, unsigned short length,
        unsigned char *data)
{
//    SEGGER_RTT_printf(0, "mpu_read_mem: mem_addr=%x - length=%x - data=%x\r\n", mem_addr, length, data);
    unsigned char tmp[2];
...
//    SEGGER_RTT_printf(0, "mpu_load_firmware: length=%d - firmware=%x - start_addr=%x - sample_rate=%d\r\n", length, firmware, start_addr, sample_rate);
    
    if (!firmware)
        return -1;
    for (ii = 0; ii < length; ii += this_write) {
        this_write = min(LOAD_CHUNK, length - ii);
        //SEGGER_RTT_printf(0, "ii: %d\r\n", ii);
        //SEGGER_RTT_printf(0, "this_write: %d\r\n", this_write);
